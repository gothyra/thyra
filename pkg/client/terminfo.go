...
func (c *Client) setup_term_builtin() error {
	name := os.Getenv("TERM")
	if name == "" {
...
func (c *Client) setup_term() (err error) {
	var data []byte
	var header [6]int16
	...
	for i, _ := range c.keys {
		c.keys[i], err = ti_read_string(rd, str_offset+2*ti_keys[i], table_offset)
		if err != nil {
			return
		}
	}
	...
	for i, _ := range c.funcs[:len(c.funcs)-2] {
		c.funcs[i], err = ti_read_string(rd, str_offset+2*ti_funcs[i], table_offset)
		if err != nil {
			return
		}
	}
	c.funcs[tMaxFuncs-2] = ti_mouse_enter
	c.funcs[tMaxFuncs-1] = ti_mouse_leave
	return nil
}
